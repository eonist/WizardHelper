 ...
 * - Fixme: ⚠️️ Make typealias for closures and org better, also add support for custom filetypes etc
 */
...
extension WizardHelper {
    ...
    * - Fixme: ⚠️️ The bellow requires access to a viewController, so probably move it to `ViewController.swift` and use event to propagate, or check if there is a method to get cur viewconteoller from the POV of view, or pov of current app state
